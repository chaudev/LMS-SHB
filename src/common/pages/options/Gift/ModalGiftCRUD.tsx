...
export const ModalGiftCRUD: React.FC<I> = ({ mode, dataRow, onRefresh, setOpen }) => {
	...
	const handleCreate = async (data) => {
		try {
			setIsLoading(true)
			const res = await giftApi.add(data)
			if (res.status === 200) {
	...
	const handleUpdate = async (data) => {
		try {
			setIsLoading(true)
			const res = await giftApi.update(data)
			if (res.status === 200) {
				ShowNoti('success', res.data.message)
				onRefresh && onRefresh()
				form.resetFields()
	...
	const handleRemove = async (data) => {
		try {
			setIsLoading(true)
			const res = await giftApi.delete(data?.Id)
			if (res.status === 200) {
				ShowNoti('success', res.data.message)
				onRefresh && onRefresh()
				onClose()
							...
							{mode != 'delete' && (
								<>
									<div className="col-span-2">
										<UploadImageField form={form} name="Thumbnail" label="Ảnh quà tặng" />
									</div>
									<div className="col-span-2">
										<InputTextField
											isRequired
