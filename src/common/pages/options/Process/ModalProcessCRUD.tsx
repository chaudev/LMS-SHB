...
export const ModalProcessCRUD: React.FC<I> = ({ mode, dataRow, onRefresh, setOpen }) => {
	...
	const handleCreate = async (data) => {
		try {
			setIsLoading(true)
			const res = await processApi.add(data)
			if (res.status === 200) {
	...
	const handleUpdate = async (data) => {
		try {
			setIsLoading(true)
			const res = await processApi.update(data)
			if (res.status === 200) {
				ShowNoti('success', res.data.message)
				onRefresh && onRefresh()
				form.resetFields()
	...
	const handleRemove = async (data) => {
		try {
			setIsLoading(true)
			const res = await processApi.delete(data?.Id)
			if (res.status === 200) {
				ShowNoti('success', res.data.message)
				onRefresh && onRefresh()
				onClose()
