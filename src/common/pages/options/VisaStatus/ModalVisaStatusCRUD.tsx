...
export const ModalVisaStatusCRUD: React.FC<I> = ({ mode, dataRow, onRefresh, setOpen }) => {
	...
	const handleCreate = async (data) => {
		try {
			setIsLoading(true)
			const res = await visaStatusApi.add(data)
			if (res.status === 200) {
	...
	const handleUpdate = async (data) => {
		try {
			setIsLoading(true)
			const res = await visaStatusApi.update(data)
			if (res.status === 200) {
				ShowNoti('success', res.data.message)
				onRefresh && onRefresh()
				form.resetFields()
	...
	const handleRemove = async (data) => {
		try {
			setIsLoading(true)
			const res = await visaStatusApi.delete(data?.Id)
			if (res.status === 200) {
				ShowNoti('success', res.data.message)
				onRefresh && onRefresh()
				onClose()
