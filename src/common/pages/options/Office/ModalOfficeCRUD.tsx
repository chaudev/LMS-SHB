...
export const ModalOfficeCRUD: React.FC<I> = ({ mode, dataRow, onRefresh }) => {
	...
	const handleCreate = async (data) => {
		try {
			setIsLoading(true)
			const res = await officeApi.add(data)
			if (res.status === 200) {
	...
	const handleUpdate = async (data) => {
		try {
			setIsLoading(true)
			const res = await officeApi.update(data)
			if (res.status === 200) {
				ShowNoti('success', res.data.message)
				onRefresh && onRefresh()
				form.resetFields()
	...
	const handleRemove = async (data) => {
		try {
			setIsLoading(true)
			const res = await officeApi.delete(data?.Id)
			if (res.status === 200) {
				ShowNoti('success', res.data.message)
				onRefresh && onRefresh()
				onClose()
						...
						<div className="grid grid-cols-2 gap-x-4 antd-custom-wrap">
							...
							{mode != 'delete' && (
								<>
									...
									<div className="col-span-2">
										<InputTextField label="Địa chỉ" name="Address" placeholder="Nhập địa chỉ" />
									</div>
								</>
							)}
						</div>
