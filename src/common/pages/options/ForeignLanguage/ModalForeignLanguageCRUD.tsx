...
export const ModalForeignLanguageCRUD: React.FC<I> = ({ mode, dataRow, onRefresh, setOpen }) => {
	...
	const handleCreate = async (data) => {
		try {
			setIsLoading(true)
			const res = await foreignLanguageApi.add(data)
			if (res.status === 200) {
	...
	const handleUpdate = async (data) => {
		try {
			setIsLoading(true)
			const res = await foreignLanguageApi.update(data)
			if (res.status === 200) {
				ShowNoti('success', res.data.message)
				onRefresh && onRefresh()
				form.resetFields()
	...
	const handleRemove = async (data) => {
		try {
			setIsLoading(true)
			const res = await foreignLanguageApi.delete(data?.Id)
			if (res.status === 200) {
				ShowNoti('success', res.data.message)
				onRefresh && onRefresh()
				onClose()
